...
def generate_particles(count: int) -> None:
    ...
    for _ in range(max(1, count)):
        rr = sample_r(int(n), int(l))
        theta = sample_theta(int(l), int(m))
        phi = sample_phi()
        x, y, z = spherical_to_cartesian(rr, theta, phi)
        cr, cg, cb, ca = inferno_color(rr, theta, phi, int(n), int(l), int(m), scale=LIGHTING_SCALER)
...
def main() -> None:
    ...
    win = glfw.create_window(WIDTH, HEIGHT, "Quantum Simulation - Raytraced style (python)", None, None)
    if not win:
        glfw.terminate()
    ...
    while not glfw.window_should_close(win):
        ...
        glPointSize(max(1.0, ELECTRON_R * 6.0))
        glBegin(GL_POINTS)
        for p in particles:
            rr = math.sqrt(p.x * p.x + p.y * p.y + p.z * p.z)
            if rr > 1e-6:
                theta = math.acos(max(-1.0, min(1.0, p.y / rr)))
                vx, vy, vz = probability_flow(p.x, p.y, p.z, int(m))
